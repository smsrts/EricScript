...
my $myscriptname = $myscript; $myscriptname =~ s%.*/%%g;
...
if ($printdb != 0) {
    system("R --slave --args $ericscriptfolder,$dbfolder,$ensversion,$printdb < $ericscriptfolder/lib/R/RetrieveRefId.R");
	exit(100);
}
...
#	}
	...
	if (! -d "$outputfolder/aln") { mkdir("$outputfolder/aln"); }
	if (! -d "$outputfolder/out") { mkdir("$outputfolder/out"); }
	$verbose ||= 0;			
	$minreads ||= 3;
	$ntrim ||= -1;
	$removetemp ||= 0;
	...
	my $varfile = "$outputfolder/ericscript.vars";
	...
	system("bash", "$ericscriptfolder/lib/bash/RunEric.sh", "$outputfolder");
} elsif ($checkdb != 0) {
	if (-d $dbfolder) {
